...
- **2026-09-19** 评测层落地（D4）：题库模型 + 程序级校验 + **gold 三道独立闸门** + 判分执行器；
  自建 A 股题库 29 题，**基线 21/29（72.4%）**，一次评测逼出并修掉 7 个此前未知的缺陷
...
│   ├── retrieval.py      # 四路检索 + RRF 融合 + 可插拔 Embedder（强弱通道分离）
...
| 应拒答正确 | 3 / 7（误答 4） |
| `ABORT` | 0 |
| 按题型 | 单值抽取 14/15 ｜ 勾稽计算 4/5 ｜ 综合判断 0/2 ｜ 应拒答 3/7 |
| 按口径 | 合并 18/19 ｜ 母公司 0/2 ｜ 跨口径 0/1 |
| gold 标注自检 | **29 / 29 通过** |
...
失败分析、7 个由评测逼出的缺陷、以及"证明了什么 / 没证明什么"，见 `docs/评测基线-v1.0.md`。
...
| `docs/评测基线-v1.0.md` | 实测结果：72.4% 基线、失败逐条分析、由评测逼出的 7 个缺陷、证明了什么 / 没证明什么 |
| `docs/技术选型-v1.1.md` | 选型的唯一权威来源，含每层候选对比与实测依据 |
| `docs/技术问题留档.md` | P-001 ~ P-022：跨页表格、附注列、端点无 embedding、科目名口径、编排层红线冲突、评测暴露的 7 个缺陷等的设计级归档（含每条的原方案 / 短板 / 实测证据 / 解决方案 / **残留限制**） |
...
| D4 | 评测层：题库 / 自检 / 判分执行器 + **自建 A 股集 29 题 + 基线 72.4%** | 🟢 评测层已完成（FinanceBench 50 题待接） |
...
1. **题库最大的风险不是漏题，是标错。** gold 标错会把系统的**正确行为判成错误**（假阳性），
   ...
   只有它通过，才能排除「解析器错了、标注跟着一起错」这种自洽型错误。
2. **拒答护栏目前只判「有没有召回证据」，不判「召回到的证据对不对得上问句」。**
   基线里有一题被问到「现金及现金等价物净增加额」，系统返回的却是「投资活动现金流入小计」——
   数值真实、页码正确、span 校验通过，**现有全部护栏都拦不住**。
   这是下一阶段最优先要补的一类通道。
