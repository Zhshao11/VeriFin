...
| 测试模块 | 数量 | 覆盖对象 |
|---|---|---|
| `test_agent.py` | 42 | 图结构合法性、路线分流、六元组装配、拒答、预算终止、降级标记、轨迹回放与并发写 |
| `test_formulas.py` | 40 | 勾稽公式、容差推导、三级结论、必需操作数（F4 不含派生左值） |
| `test_eval.py` | 34 | 题库程序级校验、口径一致性、判定型题必须带操作数、gold 三道闸门、判分口径（操作数先于判定 / `ABORT` ≠ `REFUSE`） |
...
**Track B 基线（贵州茅台 2024 年报，29 题，零 LLM）：**
...
| 应拒答正确 | 7 / 7（误答 0） |
| `ABORT` | 0 |
| 按题型 | 单值抽取 15/15 ｜ 勾稽计算 5/5 ｜ 综合判断 2/2 ｜ 应拒答 7/7 |
| 按口径 | 合并 19/19 ｜ 母公司 2/2 ｜ 跨口径 1/1 |
| 端到端耗时 | P50 21 ms ｜ P95 25 ms |
| 证据召回率 | 22 / 22 |
| gold 标注自检 | **29 / 29 通过** |
...
> [!NOTE]
> **100% 只代表"这 29 题、这份文档、这条确定性链路"的全覆盖，不代表系统不犯错。**
> 它没证明的同样重要：拒答率样本只有 7 题（其中 2 题是解析窗口人为造成）、
