...
```bash
.venv/bin/python -m pytest                      # 352 项测试
...
- **2026-09-21** 口语简称归一（P-030 两层根治）：新增 `verifin/aliases.py`，把「经营现金流」
  这类口语简称在检索前映射到规范科目名；web 与 Agent 共用同一份歧义判定
  （`guards.detect_ambiguous_abbreviation`），并补上 web `/_api/ask` **原先缺失的
  `label_consistency` 关卡** —— 静默错答的直接成因是那条路没有兜底。
  题库扩至 33 题（+4 题守 P-030），352 项测试全绿
- **2026-09-19** 护栏与能力补全（D5）：基线从 **21/29（72.4%）** 提到 **29/29（100%）**。
  补齐四类拒答护栏（**问答一致性 / 主体约束 / 期间约束 / 口径歧义**）+ 新增「**两行相减**」动作，
  共 8 个工具；端到端耗时 P50 21ms / P95 25ms，证据召回率 22/22
- **2026-09-19** 报表**口径消歧**：chunk 携带合并 / 母公司标签，取数前统一消歧。
  同名科目在两套报表里数值不同、且都是真数字 —— 不消歧会安静地给出另一张表的数字
- **2026-09-19** 评测层落地（D4）：题库模型 + 程序级校验 + **gold 三道独立闸门** + 判分执行器；
  自建 A 股题库 29 题，一次评测逼出并修掉 7 个此前未知的缺陷
- **2026-09-19** 判分口径加固：**判定型题先比操作数、再比判定** ——
  同一恒等式在合并与母公司两套口径下都能平衡，只看 `PASS` 会把「取到真数、答错题」判成对
- **2026-09-19** Agent 编排落地（D3）：显式图 + 三重预算 + 执行轨迹入库。
...
>
> 但只有这三条还不够（评测发现）：**LLM 不撒谎 ≠ 系统答对了**。
> 「取到真数、答错题」是另一类错误，需要第四层护栏（见下）来堵。
...
**第四层：确定性护栏**（`verifin/guards.py` + `verifin/scope.py`）

前三层管的是「这个数是不是原文里的」。**第四层管的是「这个数是不是问句问的」** ——
这是评测逼出来的一层，前三层对它无能为力：

| 护栏 | 挡的错误 | 为什么前三层拦不住 |
|---|---|---|
| **问答一致性** | 问 A 科目，答成 B 科目 | 数值真、页码真、span 过 —— 全都在校验"数在不在原文"，没在校验"数对不对得上问题" |
| **口径歧义** | 「资产总计是多少？」（两套口径数值不同） | 两套都是真数字。不消歧会安静地给出其中一张表的数 |
| **主体约束** | 问「宁德时代」却答茅台的值 | 科目名匹配成功，检索层不知道自己在为哪份文档服务 |
| **期间约束** | 问「2025 年」却答 2024 年值 | 同上 |

这四道闸的取舍**刻意不同**：主体/期间约束**判断不了就放行**（误拦会把正确回答变拒答），
**问答一致性判断不了就拒答**（给错一个财务数字，后果不是"体验差"）。
...
十二个节点（其中 3 个是终态）、八个工具、一张邻接表。图是**声明式**的（`GRAPH` 字典就是图本身），
所以既能渲染出来给人看，也能被单测逐条校验（后继是否都存在、终态是否真的无后继）。

**`GUARD` 是回答前的第一道闸**：主体 / 期间约束不过，后面做什么都是错的。
**`DIFF` 是独立动作**：「X 与 Y 的差额」既不是「取一行」也不套公式，需要单独一条路径。
...
3. **`ANSWER` 还要过「问答一致性」**：取到的行必须是问句点名的那一行，
   否则 `LABEL_MISMATCH` 拒答。这道闸挡的是最阴的一类错误 ——
   数值真实、页码正确、span 校验通过，但**答的是另一个科目**。
...
D4 的评测跑完一次基线，**逼出并修掉了 7 个此前不知道的缺陷**（P-016 ~ P-022），
通过率从 41.4% 一路到 72.4%。其中三个尤其典型 —— 它们让**本就该工作的东西处于关闭状态**：

| 缺陷 | 静默失效的表现 |
|---|---|
| 弱通道（TF-IDF 向量）能单独产生召回 | 停用词"的"就能让无关文档拿到 0.40 相似度 → **掏空拒答护栏** |
| label 通道只做整串相等 | 真实问句永远不命中 → 主召回通道形同虚设 |
| 公式触发表只覆盖 F1 | F2a / F2b / F3a / F4 / F6 永远走不到 |

**它们此前都没被任何单元测试发现** —— 因为单元测试都在问"给定输入有没有正确输出"，
**没有一个测试问"这条通道在真实问句上还会不会触发"**。这正是评测存在的理由。

D5 又逼出 4 条同类缺陷（P-023 ~ P-026），其中两条值得单说：

| 缺陷 | 为什么危险 |
|---|---|
| **口径不消歧**（P-023） | 同名科目在两套报表里数值不同、都是真数字。更糟的是**同一恒等式在两套口径下都能平衡** —— 取错口径在勾稽路径上表现为 `PASS` |
| **问答一致性不校验**（P-024） | 数值真、页码真、span 过，却**答的是另一个科目**。span 校验 / Decimal / 页码溯源**一道都拦不住**，它洞穿的是"证据可溯源"这个核心承诺 |

补完这 4 条 + 新增「两行相减」动作后，基线从 72.4% 到 **100%** —— 
**这 +27.6 个百分点全部来自"补齐本就该有的护栏"，不是"提升模型能力"**。

评测层自己也守同一条纪律：**gold 标注自检有三道独立闸门**，
其中一道（G3）只读**原始 PDF**，是唯一独立于本项目解析产物的 ——
只有它通过，才能排除"解析器错了、标注跟着一起错"这种自洽型错误。
...
│   ├── lexicon.py        # 财务专用词典（jieba 不认识会计科目，必须注入）+ 其他发行人清单
│   ├── scope.py          # 报表口径原语（合并 / 母公司识别，含「归属于母公司」陷阱）
│   ├── guards.py         # 确定性护栏：问答一致性 + 主体/期间约束
│   ├── retrieval.py      # 四路检索 + RRF 融合 + 可插拔 Embedder（强弱通道分离）+ chunk 带口径
│   ├── runtime.py        # 文档运行时装配：解析产物 → chunk → 索引 → PDF 句柄（单一入口）
│   ├── agent/            # Agent 编排层 —— 显式图，不依赖任何 Agent 框架
│   │   ├── graph.py      #   12 节点 + 邻接表 + 预算三件套 + 参数由图填 + 意图分流
│   │   ├── planner.py    #   LLM 选节点 + 确定性兜底（降级标 source="policy"）
│   │   ├── tools.py      #   8 个工具 + ToolRuntime 资源容器 + 口径消歧的单一落点
│   │   └── trace.py      #   执行轨迹入库（SQLite），逐步可回放
│   ├── eval/             # 评测层
│   │   ├── schema.py     #   题目模型 + 程序级校验（口径一致性 / 判定型必须带操作数）
│   │   ├── bank.py       #   题库 JSONL 读写 + 构成统计
│   │   ├── docs.py       #   文档登记表（PDF / 索引 / 解析窗口 三件套路径）
│   │   ├── gold_check.py #   gold 三道独立闸门（索引页 / 片段 / **原始 PDF**）
│   │   └── run_eval.py   #   判分执行器：操作数先于判定、ABORT ≠ REFUSE、降级率读轨迹
...
│   ├── build_eval_bank.py# 生成 Track B 题库（gold 从索引原样取，不手抄）
...
| 测试模块 | 用例数 | 覆盖对象 |
|---|---|---|
| `test_agent.py` | 84 | 图结构合法性、路线分流、六元组装配、拒答、歧义简称预检、预算终止、降级标记、轨迹回放与并发写 |
| `test_aliases.py` | 47 | 口语简称归一：确定映射、歧义检测、最长优先、词尾保护（报表名/小计不被误咬）、单趟扫描 |
| `test_eval.py` | 44 | 题库程序级校验、口径一致性、判定型题必须带操作数、gold 三道闸门、判分口径（操作数先于判定 / `ABORT` ≠ `REFUSE` / 拒答原因比对） |
| `test_formulas.py` | 40 | 勾稽公式、容差推导、三级结论、必需操作数（F4 不含派生左值） |
| `test_tables.py` | 32 | 双序列化解析、跨页拼接、数值列推导 |
| `test_normalize.py` | 27 | 单位换算、全角半角、千分位 |
| `test_retrieval.py` | 26 | 词典、四路召回、RRF、端到端、**强弱通道分离**（弱通道不得单独产生召回） |
| `test_geometry.py` | 21 | 容错匹配、折行标签、同行校验、页码边界 |
| `test_span.py` | 20 | span 硬校验两层关卡 |
| `test_adversarial.py` | 11 | Track D 对抗输入 + **反向对照**（证明 7/7 不是假绿） |

**合计 352 项，全部通过。**（用例数为 `pytest --collect-only` 展开后的实测值，含参数化用例。）
...
```bash
.venv/bin/python scripts/build_eval_bank.py --dry-run   # 题库格式与口径一致性校验
.venv/bin/python -m verifin.eval.gold_check             # gold 标注自检（三道独立闸门）
.venv/bin/python -m verifin.eval.run_eval               # 跑基线（确定性策略，不需要 LLM）
.venv/bin/python -m verifin.eval.run_eval --planner llm # 换 LLM 调度器再跑一遍
```

**Track B 基线（贵州茅台 2024 年报，33 题，零 LLM）：**
...
| 应拒答正确 | 8 / 8（误答 0） |
| `ABORT` | 0 |
| 按题型 | 单值抽取 18/18 ｜ 勾稽计算 5/5 ｜ 综合判断 2/2 ｜ 应拒答 8/8 |
| 按口径 | 合并 22/22 ｜ 母公司 2/2 ｜ 跨口径 1/1 |
| 端到端耗时 | P50 22.2 ms ｜ P95 28.0 ms |
| 证据召回率 | 25 / 25 |
| gold 标注自检 | **33 / 33 通过** |

> [!IMPORTANT]
> **判分口径选择先比操作数、再比判定。**
> 同一恒等式在合并与母公司两套口径下**都能平衡**（差额都是 0.00）。
> 只看 `PASS` 的话，用母公司数据去算合并恒等式**会被判成对** —— 这正是本项目最想防的错误。
>
> **`ABORT` 不算拒答成功。** `ABORT` 是「没算完」，`REFUSE` 是「证据不足」；
> 把前者算成后者，等于把规划器缺陷伪装成护栏生效。
>
> **R 题还比"为什么拒答"**（`expected_refusal_code`）。修复前后系统可能都会拒答，
> 只是原因不同；只比"有没有拒答"的话，一个把所有问句都拒答的废系统也能拿满分。

失败分析、11 个由评测逼出的缺陷、以及"证明了什么 / 没证明什么"，见 `docs/评测基线-v1.0.md`。

> [!NOTE]
> **100% 只代表"这 33 题、这份文档、这条确定性链路"的全覆盖，不代表系统不犯错。**
> 它没证明的同样重要：拒答率样本只有 8 题（其中 2 题是解析窗口人为造成）、
> 未做并发压测、六元组的公司/期间仍来自演示配置、LLM 调度器一次都没被调用。
> 逐条列在基线文档的"没证明什么"一节。
...
| `docs/评测方案-v1.0.md` | 评测怎么测：三轨设计、题型分层、gold 三道闸门、判分口径、指标体系、复现命令 |
| `docs/评测基线-v1.0.md` | 实测结果：100% 基线、D4→D5 的能力缺口逐条处置、由评测逼出的 11 个缺陷、证明了什么 / 没证明什么 |
| `docs/技术选型-v1.1.md` | 选型的唯一权威来源，含每层候选对比与实测依据 |
| `docs/技术问题留档.md` | P-001 ~ P-026：跨页表格、附注列、端点无 embedding、科目名口径、编排层红线冲突、口径消歧、问答一致性、主体/期间约束、两行相减等的设计级归档（含每条的原方案 / 短板 / 实测证据 / 解决方案 / **残留限制**） |
...
| D4 | 评测层：题库 / 自检 / 判分执行器 + **自建 A 股集 29 题**（基线 72.4%，暴露 7 个缺陷） | ✅ 已完成 |
| D5 | 护栏与能力补全：口径消歧 / 问答一致性 / 主体·期间约束 / 两行相减 → **基线 100%** | ✅ 已完成 |
| D6 | FinanceBench 50 题（Track A）+ 封面结构化解析 | ⬜ 下一步 |
| D7 | V0 / V1 / V2 三版 + 消融实验 | ⬜ 待开始 |
| D8 | 失败案例归档与演示 | ⬜ 待开始 |

评测指标：单值核验准确率 / 勾稽核验准确率 / 综合核验准确率 / **拒答率（单独报）** / 证据召回准确率 / 引用支持度 / 端到端耗时 P50·P95 / 单次查询成本。
另加两项本层自设指标：**降级率**（读轨迹 `source` 字段）与 **`ABORT` 率**（与拒答率分开统计）。
...
**D4 自身留下的两条纪律**（由 29 题基线暴露）：

1. **题库最大的风险不是漏题，是标错。** gold 标错会把系统的**正确行为判成错误**（假阳性），
   比漏题危险。所以有 gold 自检，且其中一道闸门只看**原始 PDF** ——
   只有它通过，才能排除「解析器错了、标注跟着一起错」这种自洽型错误。
2. **拒答护栏不能只判「有没有召回证据」，必须判「召回到的证据对不对得上问句」。**
   基线里有一题被问到「现金及现金等价物净增加额」，系统返回的却是「投资活动现金流入小计」——
   数值真实、页码正确、span 校验通过，**旧护栏全部拦不住**。D5 已补（`label_consistency`）。

**D5 留下的三条纪律**：

3. **护栏的判别力必须延伸到「问答一致性」**：它不属于"提高准确率"，属于"堵住把错误答案
   伪装成正确答案的通道"。在一个输出财务数字的系统里，这类洞比"答不出"危险得多。
4. **同类科目两套口径是最难自动发现的错误**：同一恒等式在合并与母公司下**都能平衡**（差额 0.00），
   所以判分要**先比操作数再比判定**，取数要**先消歧口径再取行** —— 两道防线缺一不可。
5. **两类护栏的取舍要分开定**：主体/期间约束判断不了就**放行**（误拦会把正确回答变拒答）；
   问答一致性判断不了就**拒答**（给错一个财务数字，后果不是"体验差"）。
...
11. **六元组的「公司 / 期间」目前取自演示配置**，不是从 PDF 封面解析出来的。
    ...
    为什么不在只有一份年报时就动手：按封面文本写公司名提取规则，
    面临"正则误抓会计师事务所或子公司名"的静默取错风险（项目里 P-008 / P-009 都是这类），
    而**单一文档无法验证一条提取规则**（n=1 规则在本项目里不视为已验证）。
    这是 D4 引入更多年报后的第一件事，不是悬而未决的遗留。
...
仓库内的 `data/financebench_doc_info.jsonl` 是 FinanceBench 的**文件名清单**
（仅 `doc_name` / 公司 / 行业 / 文档类型 / 期间 / 原始链接，不含题目与答案），
随仓库分发时同样按 CC-BY-NC-4.0 约束，**不得用于商业用途**。
`data/cninfo.json` 与 `data/orgid.json` 是巨潮资讯公开接口的响应缓存。
